					...
					for k in range(stockStart[i],endIndex):
						...
						if dates[k]==today:
							...
							if ((pd.to_numeric(bid)>0).bool()):
								#commission fee is 0.0015(0.15%)
								#allocationVector[i]=allocationVector[i]*0.9985
								amountVector[i]=np.floor(float(allocationVector[i])/(100*bid))
							else:
								amountVector[i]=0
							#print "amountVector[%d]:%.6f" % (i,amountVector[i])
							allocationVector[i]=allocationVector[i]-amountVector[i]*100*bid*1.0015
...
if __name__=="__main__":
	...
	dataSrc='/Users/xinyuan/Desktop/codeForPaper/Step2_select_best_factor_combination/6JLRto3PB_portfolios/'
	initialFlag=False
	lastFlag=False
	prevClosing=[]
	currentClosing=[]
